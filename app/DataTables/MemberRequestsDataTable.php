...
class MemberRequestsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('action', 'admin.members.requests_datatables_actions')
            ->addIndexColumn()
            ->addColumn('Sl', '')
            ->addColumn('about', function ($dataTable) {
                return Str::limit($dataTable->about, 50);
            })
            ...
            ->rawColumns(['facebook','twitter','linkedin', 'action', 'image', 'about']);
    }

    public function query(Member $model)
    {
        return $model->newQuery()->where('status',0)->latest();
    }
